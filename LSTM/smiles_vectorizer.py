...
class SmilesIterator(Iterator):
    ...
    def next(self):
        ...
        indxs = [x for x in range(len(batch_y)) if batch_y[x] == 1]

        size_diff = len(batch_y) - (2 * len(indxs))

        for _ in range(size_diff):
            rand_indx = random.choice(indxs)
            batch_x = np.append(batch_x, np.expand_dims(batch_x[rand_indx], axis = 0), axis = 0)     
            batch_y = np.append(batch_y, [1])
