...
def parse_arguments():
    ...
    parser.add_argument('--vocab', '-v', type=str, default='data/chembl_vocab.pkl', help='vocabulary (.pkl)')
    parser.add_argument('--data', '-d', type=str, default='data/chembl24smiles.csv', help='train corpus (.csv)')
    ...
    parser.add_argument('--n_layer', '-l', type=int, default=2, help='number of layers')
...
def main():
    ...
    try:
        dataset = Seq2seqDataset(pd.read_csv(args.data)['SMILES'].values, vocab)
    except:
        dataset = Seq2seqDataset(pd.read_csv(args.data)['canonical_smiles'].values, vocab)
    test_size = int(0.01 * len(pd.read_csv(args.data)))
    train, test = torch.utils.data.random_split(dataset, [len(dataset)-test_size, test_size])
    train_loader = DataLoader(train, batch_size=args.batch_size, shuffle=True, num_workers=args.n_worker,collate_fn = my_collate)
    test_loader = DataLoader(test, batch_size=args.batch_size, shuffle=True, num_workers=args.n_worker, collate_fn = my_collate)
    ...
    for e in range(1, args.n_epoch):
        ...
        for b, sm in tqdm(enumerate(train_loader)):
           # print("it beginnnnnnnnnnnnnn", b)
            sm = torch.t(sm.cuda()) # (T,B)
            optimizer.zero_grad()
            output = model(sm.cuda()) # (T,B,V)
            loss = F.nll_loss(output.view(-1, len(vocab)),
                sm.contiguous().view(-1), ignore_index=PAD)
            loss.backward()
            optimizer.step()
            if b%1000==0:
                print('Train {:3d}: iter {:5d} | loss {:.10f} | ppl {:.10f}'.format(e, b, loss.item(), math.exp(loss.item())))
               # print("1k if printed")

            if b %10000 == 0:
            #if b ==99 :
                loss = evaluate(model, test_loader, vocab)
                print('Val {:3d}: iter {:5d} |  {:.10f} | ppl {:.10f}'.format(e, b, loss, math.exp(loss)))
                # Save the model if the validation loss is the best we've seen so far.
                if not best_loss or loss < best_loss:
                    print("[!] saving model...")
                    if not os.path.isdir(".save"):
                        os.makedirs(".save")
                    torch.save(model.state_dict(), './.save/trfm_NEWAID_%d_%d.pkl' % (e,b))
                    best_loss = loss
                #print("10k if saved and printed i guess")
            #print('it end')
...
if __name__ == "__main__":
    try:
        ...
        print('Total Training time : ',(end-start)/3600,' hrs')
